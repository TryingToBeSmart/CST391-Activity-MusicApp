...
/*
OkPacket is imported from the MySQL library. 
OkPacket is the type that is returned when 
status information is returned by MySQL rather 
than a data set as with the SELECT query. This 
is the mechanism for returning the automatically
(by MySQL) assigned primary key for new records. 
*/

/*
The 'readAlbums' method supports URL query strings. 
Query string work like optional parameters in a URL. 
The query string begins with a '?' and then is a set 
of key-value pairs separated by '&'. Since query 
strings are optional, the router cannot use the query 
string to differentiate between different requests. 
*/
export const readAlbums: RequestHandler  = async (req: Request, res: Response) => {
    try {
        ...
        /*
        2 options:
        If the query parameter is absent, 'albumId' will get the 
        value 'NaN', or Not a Number. If a query parameter is 
        set and a number, then albumId will be set to that number
        */
